...
for fold in "${folds[@]}"; do
	...
	for target in "${targets[@]}"; do
		version=MI03D05B_${target}_${fold}_${ensemble_models}
		job_name="$version.job"
		out_file="../eo/$version.out"
		err_file="../eo/$version.err"
		ID=$(sbatch --dependency=$2 --error=$err_file --output=$out_file --job-name=$job_name --mem-per-cpu=$memory -t $time MI03D05B_Predictions_eids.sh $target $fold $ensemble_models)
		IDs+=($ID)
	done
done
