					...
					for learning_rate in "${learning_rates[@]}"; do
						for lambda in "${lambdas[@]}"; do
							for dropout_rate in "${dropout_rates[@]}"; do
								...
								version=$target-$image_type-$preprocessing-$architecture-$optimizer-$learning_rate-$lambda-$dropout_rate-$outer_fold
								name=M03A-$version
								job_name="$name.job"
								...
								for outer_fold in "${outer_folds[@]}"; do
									path_weights="../data/model-weights_$version_$outer_fold.h5"
									if find -f path_weights; then
								...
								for fold in "${folds[@]}"; do
									path_predictions="../data/Predictions_$version_$fold.csv"
									if find -f path_predictions; then
										to_run=true
									fi
								done
