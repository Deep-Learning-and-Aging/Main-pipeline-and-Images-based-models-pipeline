...
args=( 
	...
	--phenoFile=/n/groups/patel/Alan/Aging/Medical_Images/data/GWAS_data_${target}_${organ}${debug}.tab
	--phenoCol=${organ}
	--covarFile=/n/groups/patel/Alan/Aging/Medical_Images/data/GWAS_data_${target}_${organ}${debug}.tab
	...
	--numThreads=10
	--statsFile=/n/groups/patel/Alan/Aging/Medical_Images/data/GWAS_${target}_${organ}.stats.gz
	--verboseStats
)

# Add arguments for imputed data GWAS
if [ $# -eq 2 ] || [ $debug == "" ]; then
	args+=( --bgenFile=/n/groups/patel/uk_biobank/project_52887_genetics/ukb_imp_chr{1:$NC}_v3.bgen
			--bgenMinMAF=1e-3
			--bgenMinINFO=0.3
			--sampleFile=/n/groups/patel/uk_biobank/project_52887_genetics/ukb52887_imp_chr1_v3_s487296.sample
			--noBgenIDcheck
			--statsFileBgenSnps=/n/groups/patel/Alan/Aging/Medical_Images/data/GWAS_${target}_${organ}.bgen.stats.gz 
		)
fi
