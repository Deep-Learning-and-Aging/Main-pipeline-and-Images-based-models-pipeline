	...
	for organ_id_view in "${organs_ids_views[@]}"; do
		for transformation in "${transformations[@]}"; do
			for fold in "${folds[@]}"; do
				...
				sbatch --x11 --error=$err_file --output=$out_file --job-name=$job_name --mem-per-cpu=$memory -c $n_cpu_cores --gres=gpu:1 -t $time MI09D_Plots_attentionmaps.sh $target $organ_id_view $transformation $fold
			done
		done
	done
done
