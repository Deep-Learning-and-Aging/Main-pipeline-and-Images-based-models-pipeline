...
do
version=M02-$image_type-$preprocessing-$target-$architecture-$optimizer-$learning_rate-$lambda-$dropout_rate-$outer_fold
...
sbatch --error=$err_file --output=$out_file --job-name=$job_name --mem-per-cpu=$memory -c $n_cpu_cores --gres=gpu:$n_gpus -t $time MI02_Training.sh $image_type $preprocessing $target $architecture $optimizer $learning_rate $lambda $dropout_rate $outer_fold
#else
