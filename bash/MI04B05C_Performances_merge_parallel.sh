...
for target in "${targets[@]}"; do
	for fold in "${folds[@]}"; do
		for id_set in "${id_sets[@]}"; do
			version=MI04B05C_${target}_${fold}_${id_set}
			job_name="$version.job"
			out_file="../eo/$version.out"
			err_file="../eo/$version.err"
			sbatch --error=$err_file --output=$out_file --job-name=$job_name --mem-per-cpu=$memory -c $n_cpu_cores -t $time MI04B05C_Performances_merge.sh $target $fold $id_set $ensemble_models
		done
	done
done
