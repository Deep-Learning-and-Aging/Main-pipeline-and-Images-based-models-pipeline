...
for target in "${targets[@]}"; do
	for organ in "${organs[@]}"; do
		...
		ID=$(sbatch --dependency=$1 --parsable -t $time -n 1 -c 10 --mem-per-cpu $mem_per_cpu --error=$err_file --output=$out_file --job-name=$job_name MI08C_GWAS_bolt.sh $target $organ "debug_mode")
		IDs+=($ID)
	done
done
