...
saliency = get_gradients_of_activations(self.model, Xs, y, layer_name='input_1')

# grad cam
last_conv_layer = 'mixed10'
weights = \
    get_gradients_of_activations(self.model, Xs, y, layer_name=last_conv_layer,)[last_conv_layer].mean(axis=(1, 2))
weights /= weights.max() + 1e-7  # for numerical stability
activations = get_activations(self.model, Xs, layer_name=last_conv_layer)[last_conv_layer]
gradcam = np.einsum('il,ijkl->ijk', weights, activations)
# Expand the gradcam map to match the dimension of the input
zoom_factor = [1] + list(np.array(Xs[0].shape[1:3]) / np.array(grad_cam.shape[1:]))
gradcam = zoom(gradcam, zoom_factor)
# Normalize
gc_min, gc_max = gradcam.min(), gradcam.max()
gradcam = (gradcam - gc_min) / (gc_max - gc_min + 1e-7)
