...
#options
#always recalculate the data_features, VS. load them if already exist. Should be true to use same train/val/split on all models
...
if len(sys.argv) != 9:
    ...
    sys.argv = ['']
    sys.argv.append('PhysicalActivity_90001_main') #image_type
...
image_type, organ, field_id, view, transformation, target, architecture, optimizer, learning_rate, weight_decay, dropout_rate = read_parameters_from_command(sys.argv)
...
version = target + '_' + image_type + '_' + transformation + '_' + architecture + '_' + optimizer + '_' + str(learning_rate) + '_' + str(weight_decay) + '_' + str(dropout_rate)
...
path_weights_nonreg = path_store + 'model_weights_' + target + '_' + image_type + '_' + transformation + '_' + architecture + '_' + optimizer + '_' + str(learning_rate) + '_' + str(0.0) + '_' + str(0.0) + '.h5' if regularized_model else ""
...
if (not regenerate_data_features) & os.path.exists(path_store + 'data-features_' + image_type + '_' + target + '_test.csv'):
    DATA_FEATURES = load_data_features(folds=folds_tune, path_store=path_store, image_type=image_type, target=target)
else: 
    DATA_FEATURES = generate_data_features(image_type=image_type, target=target, dir_images=dir_images, image_quality_id=image_quality_ids[organ], batch_size=batch_size)
...
if continue_training:
    initial_val_metric = model.evaluate_generator(GENERATORS['val'], steps=GENERATORS['val'].n//GENERATORS['val'].batch_size)[([loss_name] + metrics_names).index(main_metric_name)]
elif main_metric_mode == 'min':
    initial_val_metric = np.Inf
elif main_metric_mode == 'max':
    initial_val_metric = -np.Inf

#define callbacks
callbacks = define_callbacks(path_store=path_store, version=version, initial_val_metric=initial_val_metric, continue_training=continue_training, main_metric=main_metric, main_metric_mode=main_metric_mode)

#garbage collector
gc.collect()

#train the model
history = model.fit_generator(generator=GENERATORS['train'], steps_per_epoch=STEP_SIZES['train'], validation_data=GENERATORS['val'], validation_steps=STEP_SIZES['val'], use_multiprocessing = True, workers=n_cpus, epochs=n_epochs_max, class_weight=class_weights, callbacks=callbacks, verbose=2)
print('\nTHE MODEL CONVERGED!\n')
