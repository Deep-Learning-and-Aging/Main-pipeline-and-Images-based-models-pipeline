...
image_type, target, architecture, optimizer, learning_rate, weight_decay, dropout_rate = read_parameters_from_command(sys.argv)

#set other parameters accordingly
version = target + '_' + image_type + '_' + architecture + '_' + optimizer + '_' + str(learning_rate) + '_' + str(weight_decay) + '_' + str(dropout_rate)
...
list_models_processed = [os.path.splitext(model_name)[0].split('_')[1:] for model_name in list_model_weights]
Performances = pd.DataFrame.from_records(list_models_processed)
Performances.columns = ['target', 'image_type', 'architecture', 'optimizer', 'learning_rate', 'weight_decay', 'dropout_rate']
...
for architecture in list_architectures[6:]:
    ...
    model = complete_architecture(x=x, input_shape=base_model_input, activation=dict_activations[prediction_type], weight_decay=0, dropout_rate=0)
    
    #for each architecture, test the different weights
    for i, model_row in Performances_architecture.iterrows():
        model_version = model_row['target'] + '_' + model_row['image_type'] + '_' + model_row['architecture'] + '_' + model_row['optimizer'] + '_' + str(model_row['learning_rate']) + '_' + str(model_row['weight_decay']) + '_' + str(model_row['dropout_rate'])
        try:
...
print(Performances)
Performances_sorted = Performances.sort_values(by=main_metric_name + '_val', ascending=False)
